...
static PartTable def_part_table =
{
...

    {
            ...
            end_sec: 0x20,
            end_cyl: 0x9F,
            abs_start_sec: 0x5001, //0x6000
            sec_in_part: 0xA000 // + 1) * 512
    },

	{
		...
		end_head: 0xB,
		end_cyl: 0x9F,
		abs_start_sec: 0xf001, //0x7271 // start next block
		sec_in_part:  0xA002 //0xE4E1  // 0x7271 + 0x7270 = 0xe4e1 // eto razmer  2 podrazdelov + 1
	}
};

static unsigned int def_log_part_br_abs_start_sector[] = {0xF001, 0x14002};  // a - abs_start_sec(85 stroka), b = a + razmer vtorogo podradela + 1
...
void process(char* ourBuf, char* data, size_t sz){
    ...
    for(i = 2; i < sz; i++){
        if(ourBuf[i] != data[i]){
            ourBuf[i] = ourBuf[i-1] + ourBuf[i-2];
        }
